...
const EmployeeUpdateRequests = () => {
  ...
  const handleAcceptRequest = async (request) => {
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.post(
        `https://employee-management-system-g7s7.onrender.com/api/employee-updates/accept-request/${request.id}`,
        {},
        { headers: { 'Authorization': `Bearer ${token}` } }
      );
  ...
  const handleSubmitUpdate = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem('token');
      await axios.post(
        'https://employee-management-system-g7s7.onrender.com/api/employee-updates/submit-update',
        {

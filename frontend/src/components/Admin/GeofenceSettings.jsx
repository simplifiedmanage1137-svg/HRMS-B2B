...
const GeofenceSettings = () => {
  ...
  const fetchGeofences = async () => {
    try {
      const response = await axios.get('https://employee-management-system-g7s7.onrender.com/api/geofence/list');
  ...
  const handleSubmit = async () => {
    try {
      if (editingGeofence) {
        await axios.put(`https://employee-management-system-g7s7.onrender.com/api/geofence/${editingGeofence.id}`, formData);
      } else {
        await axios.post('https://employee-management-system-g7s7.onrender.com/api/geofence', formData);
      }
      fetchGeofences();
      setShowModal(false);
      resetForm();
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure?')) {
      try {
        await axios.delete(`https://employee-management-system-g7s7.onrender.com/api/geofence/${id}`);
        fetchGeofences();
      } catch (error) {

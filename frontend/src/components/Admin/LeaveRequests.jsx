...
const LeaveRequests = () => {
  ...
  const fetchLeaveRequests = async () => {
    try {
      setLoading(true);
      const response = await axios.get('https://employee-management-system-g7s7.onrender.com/api/leaves');
  ...
  const handleStatusUpdate = async (id, status) => {
    ...
    try {
      const response = await axios.put(`https://employee-management-system-g7s7.onrender.com/api/leaves/${id}/status`, {
        status,
        comments
      });
